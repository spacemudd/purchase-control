@extends('layouts.app', ['title' => trans('words.manufacturers')])

@section('header')
	<nav class="breadcrumb" aria-label="breadcrumbs">
		<ul>
			<li>
				<a href="{{ route('dashboard.index') }}" aria-current="page">
					<span class="icon is-small"><i class="fa fa-inbox"></i></span>
					<span>{{ trans('words.dashboard') }}</span>
				</a>
			</li>
			<li class="is-active">
				<a href="{{ route('manufacturers.index') }}">
					<span class="icon is-small"><i class="fa fa-cubes"></i></span>
					<span>{{ trans('words.manufacturers') }}</span>
				</a>
			</li>
		</ul>
	</nav>
@endsection

@section('content')

	<div class="columns">

		<div class="column is-4">
			<p class="title is-6">
				<b>{{ trans('words.inactive') }} {{ trans('words.manufacturer') }}</b>
			</p>

			<div class="notification is-warning">
				<p class="subtitle is-7">
					<b>{{ $inactiveManufacturers }}</b>
				</p>
			</div>
		</div>

		<div class="column is-4">
			<p class="title is-6">
				<b>{{ trans('words.active') }} {{ trans('words.manufacturer') }}</b>
			</p>

			<a href="{{ route('manufacturers.all') }}">
				<div class="notification is-success">
					<p class="subtitle is-7">
						<b>{{ $activeManufacturers }}</b>
					</p>
				</div>
			</a>
		</div>
	</div>

	<manufacturers :can-create.number="{{ Auth::user()->can('create-manufacturers') ? '1' : '0' }}"></manufacturers>

@endsection
