        ...
        <div class="dropdown-item result" v-if="paginatedRecords.data">
            <div class="information">
                ...
                <table class="table is-bordered is-hoverable is-striped is-narrow is-fullwidth is-size-7">
                    <thead>
                        <tr>
                            <th style="width:140px;">{{ $t('words.issuance-number') }}</th>
                            <th style="width:100px;">{{ $t('words.issuance-date') }}</th>
                    ...
                    <tbody>
                        <tr v-for="record in paginatedRecords.data" class="pointer" @click="recordSelected(record)">
                            <td>{{ record.ref_id }}</td>
                            <td class="has-text-right">{{ record.issuance_date_human }}</td>
    ...
    export default {
        ...
        methods: {
            recordSelected(record) {
                if(this.hyperLinked) {
                    window.document.location = this.baseUrl() + '/procedures/asset-issuances/' + record.id;
                }
