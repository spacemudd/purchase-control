        ...
        <div class="dropdown-item result" v-if="paginatedRecords.data">
            <div class="information">
                ...
                <table class="table is-bordered is-hoverable is-striped is-narrow is-fullwidth is-size-7">
                    <thead>
                        <tr>
                            <th style="width:160px;">{{ $t('words.purchase-order-number') }}</th>
                            <th>{{ $t('words.main-order-number') }}</th>
                            <th>{{ $t('words.date') }}</th>
                    ...
                    <tbody>
                        <tr v-for="record in paginatedRecords.data" class="pointer" @click="recordSelected(record)">
                            ...
                            <td>{{ record.main_order_number }}</td>
                            <td class="has-text-right">{{ record.date_human }}</td>
                            <td>{{ record.employee.code + ' - ' + record.employee.name }}</td>
                            <td>
                                <span :class="'tag tag-' + record.status" class="is-capitalized" style="height:unset;">
                                    {{ record.status }}
                                </span>
                            </td>
                        </tr>
    ...
    export default {
        ...
        methods: {
            recordSelected(record) {
                if(this.hyperLinked) {
                    window.document.location = this.baseUrl() + '/procedures/purchase-orders/' + record.id;
                }
