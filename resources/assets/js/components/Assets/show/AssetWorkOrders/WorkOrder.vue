        ...
        <table class="table is-fullwidth is-narrow is-size-7 has-text-small" v-else>
        	...
        	<tbody>
        			...
        			<tr v-for="order in workOrders">
        				<td>{{ order.id_human }}</td>
                        <td>{{ order.user.full_name }}</td>
                        <td>{{ order.created_at }}</td>
                        <td>{{ order.vendor.code + ' - ' + order.vendor.description }}</td>
                        <td>{{ order.repair_submission_date }}</td>
...
<script>
    ...
    export default {
        ...
        methods: {
            getWorkOrders() {
                axios.post(this.apiUrl() + '/assets/work-orders', {
