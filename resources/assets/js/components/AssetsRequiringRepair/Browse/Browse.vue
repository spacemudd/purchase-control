			...
			<div class="col-md-12">
				<div class="level">
					<div class="level-left">
						<simple-search search-endpoint="asset-repair"
									   target-url="/procedures/asset-repair">
						</simple-search>
					</div>
				</div>
			</div>
										...
										<template v-for="asset in assets.data">
											<tr>
												...
												<td class="text-center">
													<a :href="baseUrl() + '/procedures/asset-repair/' + asset.id"
...
<script>
	export default {
		...
		methods: {
			getAssets() {
				axios.get(this.apiUrl() + '/asset-repair/paginated/10').then(response => {
			...
			selectPage(page) {
				...
				axios.get(this.apiUrl() + '/asset-repair/paginated/10' + '?page=' + page).then(response => {
					this.assets = response.data
					this.isTableLoading = false
				}).catch(error => {
