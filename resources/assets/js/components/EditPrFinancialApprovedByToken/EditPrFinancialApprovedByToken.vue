<template>
    <div>
        <p class="transparent-highlighter-bg is-size-7" @click="edit">
            {{ old_value }}
            <span v-if="!form.f_auth_by_id"><i>[Approved by]</i></span>
        </p>
    ...
    export default {
        ...
        methods: {
            ...
            save() {
                ...
                this.form.errors = [];
                let data = {f_auth_by_id: this.form.f_auth_by_id};
                axios.put(this.url, data)
                    .then(response => {
                        this.$endLoading('SAVING_RECOMMENDED_BY');
                        this.is_editing = false;
                        this.form.f_auth_by_id = response.data.f_auth_by_id;
                        this.old_value = response.data.f_auth_by ? response.data.f_auth_by.display_name : '';
