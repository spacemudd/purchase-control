                ...
                <table class="table is-bordered is-hoverable is-striped is-narrow is-fullwidth is-size-7">
                    <thead>
                        <tr>
                            ...
                            <th>{{ $t('words.department') + ' ' + $t('words.description') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="record in paginatedRecords.data" class="pointer" @click="recordSelected(record)">
                            <td>{{ record.code }}</td>
                            <td>{{ record.department.code }}</td>
                            <td>{{ record.department.description }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    export default {
        ...
        methods: {
            recordSelected(record) {
                if(this.hyperLinked) {
                    window.document.location = this.baseUrl() + '/locations/' + record.id;
                }
